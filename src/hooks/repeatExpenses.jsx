        ...
        repeats.forEach(repeat => {
            ...
            if (today > repeat.date) {
                ...
                addRepeatedExpense({
                    ...
                    UID: session.user.id})
                if (repeat.repeat == "Daily") {
                    let newDate = new Date(originalDate.getFullYear(), originalDate.getMonth(), originalDate.getDate()+1).toLocaleDateString('en-CA')
                    repeat.date = newDate
                    handleUpdate(newDate, repeat.id)
                } else if (repeat.repeat == "Weekly") {
                    let newDate = new Date(originalDate.getFullYear(), originalDate.getMonth(), originalDate.getDate()+7).toLocaleDateString('en-CA')
                    repeat.date = newDate
                    handleUpdate(newDate, repeat.id)
                } else if (repeat.repeat == "Monthly") {
                    let newDate = new Date(originalDate.getFullYear(), originalDate.getMonth()+1, originalDate.getDate()).toLocaleDateString('en-CA')
                    repeat.date = newDate
                    handleUpdate(newDate, repeat.id)
                } else if (repeat.repeat == "Yearly") {
                    let newDate = new Date(originalDate.getFullYear()+1, originalDate.getMonth(), originalDate.getDate()).toLocaleDateString('en-CA')
                    repeat.date = newDate
                    handleUpdate(newDate, repeat.id)
                } 
            }
